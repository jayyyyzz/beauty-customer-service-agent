...
class KnowledgeSourceTests(unittest.TestCase):
    ...
    def test_english_product_query_is_not_blocked_by_language_filter(self):
        filters = agent_pipeline.build_metadata_filters(
            {"intent_level1": "product_info", "keywords": ["Almay", "price"]},
            "What is the price of Almay Smart Shade Butter Kiss Lipstick?",
        )
        self.assertNotIn("language", filters)
        self.assertTrue(any(brand.casefold() == "almay" for brand in filters.get("brand", [])))

    def test_retrieval_query_normalizes_colloquial_terms(self):
        query = agent_pipeline.build_search_query(
            "a醇跟果酸能不能同一晚叠",
            {"intent_level1": "compatibility", "keywords": ["A醇", "果酸"]},
        )
        self.assertIn("视黄醇", query)
        self.assertIn("一起使用", query)

    def test_optimized_hit_rerank_removes_duplicate_content(self):
        hits = [
            {
                "_score": 10.0,
                "_source": {
            ...
            },
            {
                "_score": 9.5,
                "_source": {
                    "document_id": "policy:c",
                    "document_type": "policy",
                    "title": "售后凭证规则",
                    "content": "请保留照片和订单凭证后联系售后",
                    "content_hash": "other",
                },
            },
        ]
        ranked = agent_pipeline.rerank_and_deduplicate_hits(
            hits,
            "过敏后怎么退货",
            {"intent_level1": "after_sale", "keywords": ["过敏", "退货"]},
            final_k=3,
        )
        self.assertEqual(len(ranked), 2)
        self.assertEqual(len({item["_source"]["content_hash"] for item in ranked}), 2)
