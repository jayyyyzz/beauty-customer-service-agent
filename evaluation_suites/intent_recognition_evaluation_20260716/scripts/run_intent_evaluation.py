...
async def recognize_deployed_with_raw(
    ...
    """镜像 recognize_intents，额外保留模型原始输出和 fallback 标志。"""
    content = await _chat_completion(
        deployed_messages(history_dialogue, question),
        json_mode=True,
        max_tokens=1600,
    )
    ...
    intents = [
        ...
        if (normalized := _normalize_intent(item)) is not None
    ]
    if intents:
        return intents, content, False
...
def clarification_metrics(results: list[dict[str, Any]], threshold: float) -> dict[str, Any]:
    ...
    for item in results:
        ...
        predicted = float(item["prediction"]["minimum_confidence"]) < threshold
...
def infer_error_type(item: dict[str, Any], threshold: float) -> str | None:
    predicted_clarify = float(item["prediction"]["minimum_confidence"]) < threshold
...
def build_metrics(results: list[dict[str, Any]], dataset_path: Path) -> dict[str, Any]:
    ...
    return {
        "metadata": {
            ...
            "temperature": "provider_default (代码未显式设置)",
