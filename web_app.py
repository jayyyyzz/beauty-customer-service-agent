...
def _chat_history(payload: ChatRequest) -> dict[str, Any]:
    return {
        "conversation_id": payload.conversation_id,
...
async def _invoke_agent(payload: ChatRequest) -> dict[str, Any]:
    return await asyncio.wait_for(
        handle_user_question(
            history_dialogue=_chat_history(payload),
...
async def chat(payload: ChatRequest, request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        return _finish_chat_result(result, route, started)
